...
public class GlitchMemoryLeak : EnemyBase
{
    ...
    // FIXED: must match EnemyBase (protected override)
    ...
    IEnumerator CastRoutine()
    {
        ...
        if (castVfx)
            Destroy(Instantiate(castVfx, transform.position, Quaternion.identity), 2f);

        if (castSfx)
            AudioSource.PlayClipAtPoint(castSfx, transform.position, 1f);
