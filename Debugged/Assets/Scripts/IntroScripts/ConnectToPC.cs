...
public class ConnectToPC : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //    hide the goal bar, fade the bottom bar, then trigger scene transition.
        if (P1Connected && P2Connected && !bothAnnounced)
        {
            ...
            // Hide the goal banner
            if (goalBar) goalBar.SetActive(false);

            // 🔥🔥 NEW LINE — fade to black + switch scenes
            FindObjectOfType<SceneTransition>().BeginTransition();
    ...
    void StopListening()
    {
        ...
        // Disable all search spots (optional)
        if (disableAllSearchAfterPC)
            DisableAllSearchSpots();

        // Hide the bottom bar and clear text
        SetBottomAlpha(0f);
        bottomPrompt?.SetText(string.Empty);
    }
