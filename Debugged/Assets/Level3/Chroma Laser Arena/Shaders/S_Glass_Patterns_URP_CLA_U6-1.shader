...
Shader "TidalFlask/Glass Patterns URP CLA U6-1"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 _Color;
			float4 _EmissionColorTint;
			float _Metallic;
			float _Roughness;
			float _EmissionPatternIntensity;
						 ...
						 ) : SV_Target
			{
				...
				float2 uv_BaseTexture2 = input.ase_texcoord7.xy;
				...
				float Occlusion = 1;
				float3 Emission = ( lerpResult7 + lerpResult9 ).rgb;
		...
		Pass
		{
			...
			half4 frag(PackedVaryings input  ) : SV_Target
			{
				...
				float2 uv_BaseTexture2 = input.ase_texcoord1.xy;
				float4 tex2DNode2 = tex2D( _BaseTexture, uv_BaseTexture2 );
				#ifdef _LRSWITCH_ON
				float staticSwitch3 = tex2DNode2.r;
				#else
...
ASEEND*/
//CHKSM=92CBC358E9A4385F9EC7E38C60024C0600F2FAD6